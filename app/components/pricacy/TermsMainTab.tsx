...
import TermsAccordionMain from "./TermsAccordionMain";

export default function TermsMainTab() {
  const [activeTab, setActiveTab] = useState(payvaTermsConditions[0].key);

  const activeCategory = payvaTermsConditions.find(
        ...
        <div className="w-full md:border-t border-gray-200 flex flex-col md:flex-row md:gap-12 items-start">
          ...
          <AnimatePresence mode="wait">
            {activeCategory && (
              ...
              >
                <TermsAccordionMain sections={activeCategory.sections} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
